...
saveDir  = './results_helmholtz_permute/';
fileName = 'results';
...
% create data
speedRange = linspace(10,100,100);
A = [];
w = [];
for ii = speedRange
    % create model
...
for ii = [1,10:10:size(A,2)]
    k = speedRange(ii);

    if ~exist([saveDir,'orig/','k',num2str(ii)], 'dir'), mkdir([saveDir,'orig/','k',num2str(ii)]); end
    % for j = [1,101:100:1001]
     for j = 1:n3
        pdeplot(model,"XYData",A(:,ii,j),"ColorBar","off","Mesh","off");
        colormap(jet)
        clim([-abs(max(A(:,ii,j))) abs(max(A(:,ii,j)))]);
        axis tight
        ax = gca;
...
for q = 1:2:length(QCell)
    ...
    % reset
    results = zeros(length(kRange),length(pRange));
    ...
    for k = kRange
        ...
        for p = pRange
            ...
            % store results
            results(kCount,pCount) = fronorm(A - AApprox);
            storage(kCount,pCount) = sum(info.rho) * (size(A,1) + size(A,2)) + size(A,3) * p;
            gammas(kCount,pCount)  = gamma;
            pCount = pCount + 1;
        end
    ...
    kk      = kron(kRange(:),ones(length(pRange),1));
...
% k1Range  = [1,10:10:100];
k1Range  = [1,10:10:100,150:50:size(A,1)];
k2Range  = 1:size(A,2);
results = zeros(length(k1Range),length(k2Range),length(pRange));
storage = zeros(size(results));
...
for k3 = pRange
    ...
    for k1 = k1Range
        ...
        for k2 = k2Range
            ...
            % store results
            results(count1,count2,count3) = fronorm(A - HApprox);

            % storage costs
            storage(count1,count2,count3) = k1 * k2 * k3 + k1 * size(A,1) + k2 * size(A,2) + k3 * size(A,3);
...
kk1 = kron(kron(ones(1,length(k2Range)),k1Range),ones(1,length(pRange)));
kk2 = kron(kron(k2Range,ones(1,length(k1Range))),ones(1,length(pRange)));
pp  = kron(kron(ones(1,length(k2Range)),ones(1,length(k1Range))),pRange);
...
for k2 = k2Range
    idx = (kk2 == k2) & (kk1 == k2);
    rr  = cat(1,rr,results(idx,:));
end

T2 = array2table(rr,'VariableNames',{'k1','k2','p','err','rel_err','comp_ratio','rel_comp_ratio'});
writetable(T2,[saveDir,'hosvd/',fileName,'_k1_k2_equal.csv'])
